# 🏥 Hospital Emergency Room Performance Analytics Dashboard

An interactive **Power BI dashboard** designed to analyze and monitor **emergency room performance**, including patient flow, wait times, satisfaction levels, demographics, and department referrals to support **data-driven healthcare decision-making**.

---

## 📊 Project Overview
Emergency rooms handle high patient volumes daily, making it critical to monitor **operational efficiency** and **patient experience**.  
This project provides a **visual analytics solution** that helps hospitals identify service delays, optimize resources, and improve patient satisfaction through actionable insights.

---

## 🎯 Objectives
- Analyze emergency room **patient flow and admission trends**  
- Monitor **average wait time** and service delays  
- Track **patient satisfaction metrics**  
- Understand **patient demographics** and department referral patterns  

---

## 📈 Key Insights
- Analyzed **464+ patient records** for monthly ER performance  
- Average patient wait time: **35.19 minutes**  
- **61% of patients attended on time**, while 39% experienced delays  
- Nearly equal distribution between **admitted and non-admitted** patients  
- Highest patient volume observed in **30–44 and 45–59 age groups**  
- **General Practice** and **Orthopedics** identified as top referral departments  

---

## 🛠️ Tools & Technologies
- **Power BI** – Dashboard development and data visualization  
- **DAX** – KPI calculations and performance measures  
- **Excel / CSV** – Data cleaning and preprocessing  
- **Data Modeling** – Relationship management and performance optimization  

---

## 📌 Dashboard Features
- Interactive **KPIs** for patient count, wait time, and satisfaction  
- **Year & Month slicers** for dynamic filtering (2023–2024)  
- Demographic analysis by **gender and age group**  
- **Department-wise** patient referral analysis  
- **Admission vs non-admission** performance insights  

---

## ⭐ Skills Demonstrated
- Data Analysis  
- Healthcare Analytics  
- KPI Development  
- Dashboard Design  
- Data Visualization  
- Business Intelligence (BI)  
- Data-Driven Decision Making  

---
